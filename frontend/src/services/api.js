...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new ApiError(
      "The BhoomiSetu API is unavailable. Please try again in a moment."
    );
  }
...
export function getRiskOverview() {
  return request("/dashboard/risk-overview");
}
...
export function predictProject(projectId) {
  return request(
    `/projects/${encodeURIComponent(projectId)}/predict`,
    {
      method: "POST",
    }
  );
